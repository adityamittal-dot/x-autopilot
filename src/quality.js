 ...
 */
export function validate(text, cfg, recent = [], fmt = {}) {
  const q = cfg.quality;
  const maxChars = fmt.maxChars ?? 275;
  const reasons = [];
  const t = (text || '').trim();

  if (!t) reasons.push('empty');
  if (t.length > maxChars) reasons.push(`too long (${t.length}>${maxChars})`);
  if (fmt.maxLines && t.split('\n').length > fmt.maxLines) reasons.push(`too many lines (>${fmt.maxLines})`);
  if (/https?:\/\/|\bwww\./i.test(t)) reasons.push('contains a link (X suppresses reach on links)');
 ...
 * Higher is better.
 */
export function score(text, cfg, fmt = {}) {
  const t = text.trim();
  let s = 0;
  ...
  if (firstLine.length <= 70) s += 8;                            // tight hook
  else if (firstLine.length <= 100) s += 3;
  if (/\n/.test(t)) s += 3;                                      // line breaks scan better on X
